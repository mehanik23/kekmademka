...
EOF
  ...
  if [ "$DEVICE_ROLE" = "HQ-RTR" ]; then
    ip route add 192.168.200.0/24 via 10.10.10.2 dev gre0 2>/dev/null
  fi
...
check_gre_tunnel() {
  ...
  if ip link show gre0 &> /dev/null; then
    ...
    if [ "$DEVICE_ROLE" = "HQ-RTR" ]; then
      echo "Ping 10.10.10.2 (BR-RTR):"
      ping -c 3 -W 2 10.0.0.2
    else
      echo "Ping 10.10.10.1 (HQ-RTR):"
...
fix_gre_tunnel() {
  ...
  if [ "$DEVICE_ROLE" = "HQ-RTR" ]; then
    log "Перенастройка IP адреса 10.10.10.1/30..."
    ...
    ip route add 192.168.200.0/24 via 10.10.10.2 dev gre0
  else
    ...
    ip route add 192.168.100.0/24 via 10.10.10.1 dev gre0
  fi
...
quick_gre_test() {
  ...
  if [ "$DEVICE_ROLE" = "HQ-RTR" ]; then
    ping -c 1 -W 1 10.10.10.1 || echo "Ошибка пинга локального адреса"
  else
    ping -c 1 -W 1 10.10.10.2 || echo "Ошибка пинга локального адреса"
  fi
  echo ""
  ...
  if [ "$DEVICE_ROLE" = "HQ-RTR" ]; then
    ping -c 1 -W 2 10.10.10.2 || echo "Ошибка пинга удаленного адреса"
  else
    ping -c 1 -W 2 10.10.10.1 || echo "Ошибка пинга удаленного адреса"
  fi
  echo ""
...
main_menu() {
  ...
  while true; do
    clear
    echo -e "${BGREEN}=========================================${NC}"
    echo -e "${BGREEN}   ПОРВАННЫЙ ПРОВЕРЯЮЩИЙ $DEVICE_ROLE${NC}"
    echo -e "${BGREEN}=========================================${NC}"
